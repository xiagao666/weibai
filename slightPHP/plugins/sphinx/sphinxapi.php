...
define("SPH_RANK_WORDCOUNT",
    3);    ///< simple word-count weighting, rank is a weighted sum of per-field keyword occurence counts
...
class SphinxClient
{
    ...
    function SetOverride($attrname, $attrtype, $values)
    {
        ...
        assert(in_array($attrtype,
            array(SPH_ATTR_INTEGER, SPH_ATTR_TIMESTAMP, SPH_ATTR_BOOL, SPH_ATTR_FLOAT, SPH_ATTR_BIGINT)));
        assert(is_array($values));
    ...
    function AddQuery($query, $index = "*", $comment = "")
    {
        ...
        // build request
        $req = pack("NNNNN", $this->_offset, $this->_limit, $this->_mode, $this->_ranker,
            $this->_sort); // mode and limits
    ...
    function BuildExcerpts($docs, $index, $words, $opts = array())
    {
        ...
        $req .= pack("NNN", (int)$opts["limit_passages"], (int)$opts["limit_words"],
            (int)$opts["start_passage_id"]); // v.1.2

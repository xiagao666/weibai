...
class SRoute
{
    ...
    static function setConfigFile($file)
    {
        ...
        self::$_Routes = array_merge(self::$_Routes, (array)parse_ini_file(self::$_RouteConfigFile, true));
        self::parse();
    }
    ...
    private static function parse()
    {
        ...
        if (self::$_Routes) {
            foreach (self::$_Routes as $route) {
                ...
                if (preg_match_all("/$pattern/sm",
                    !empty($_SERVER['PATH_INFO']) ? $_SERVER['PATH_INFO'] : $_SERVER['REQUEST_URI'], $_m)) {
                    array_shift($_m);
                    $params = array();
                    if (!empty($_m)) {
                        foreach ($_m as $_m2) {

...
class Tpl
{
    ...
    static function fetch($tpl)
    {
        ...
        $compiled_file = self::$compile_dir . "/" . base64_encode($tpl) . ".%%.html";
    ...
    private static function _match($matches)
    {
        ...
        $content = preg_replace_callback($pattern, create_function('$m',
            '$t = trim($m[3]);$v = trim($m[2]);if(empty($t)){return "{$m[1]}($v){";}else{return "{$m[1]}$v{";}'),
            $content);
